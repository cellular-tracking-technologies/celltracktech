#' Load Node Detection Data
#'
#' @param directory Directory that contains node data
...
load_node_detection_data <- function(directory,
    ...
    if (!is.null(start_time)) {
        ...
        file_df <- subset.data.frame(file_df,
                                     file_df$time >= day_before)
    }
    if (!is.null(stop_time)) {
        # The time here is upload time and not the record time so allow a day buffer here
        day_after <- stop_time + 86400
        file_df <- subset.data.frame(file_df,
                                     file_df$time <= day_after)
    }
